...
public class CategoryHandler implements ICategoryHandler {
    ...
    public CategoryPage<CategoryResponse> getCategoriesByPage(int page, int size, String sortDirection) {
        ...
        if (!(Constants.Sort.ASC.toString().equalsIgnoreCase(sortDirection) ||
            ...
            throw new InvalidParameterException(AppExceptionMessage.INVALID_SORT_DIRECTION.getMessage());
        }

        if (page < 0){
            throw new InvalidParameterException(AppExceptionMessage.NO_NEGATIVE_PAGE.getMessage());
        }

        if (size <= 0){
            throw new InvalidParameterException(AppExceptionMessage.SIZE_GREATER_ZERO.getMessage());
        }
